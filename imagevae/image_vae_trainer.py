...
MNIST_REG_TYPES = {         # 手写体的正则化属性
    "digit_identity": 0,    # 数字身份
    "area": 1,              # 面积
    "length": 2,            # 长度
    "thickness": 3,         # 厚度
    "slant": 4,             # 倾斜度
...
MNIST_NORMALIZATION_FACTORS = { # 手写体归一化因素
    "digit_identity": (0, 9),
    "area": (0, 350),
    "length": (0, 100),
    "thickness": (0, 15),
...
DATASET_REG_TYPE_DICT = {   # 数据集 和其相对应的正则化属性类型的 字典
    'mnist': MNIST_REG_TYPES,
    'dsprites': DSPRITES_REG_TYPE
}


def get_reg_dim(attr_dict):     # 获得正则化属性维度，传入属性的字典
    reg_dim = []    # 列表
    for r in attr_dict.keys():
        if r == 'digit_identity' or r == 'color':
            continue
        reg_dim.append(attr_dict[r])    # 正则化维度列表后面添加一个值，值由键r去字典里找
    reg_dim = tuple(reg_dim)    # 变成元组，不能再改变
    return reg_dim
...
class ImageVAETrainer(Trainer):
    def __init__(
            ...
            model: MnistVAE,
            lr=1e-4,        # 学习率
            reg_type: Tuple[str] = None,    # 正则化的属性名称
            reg_dim: Tuple[int] = 0,        # 正则化的属性维度
            dec_dist='bernoulli',           # 解码器的分布（伯努利）
            beta=4.0,       # KLD loss的加权参数
            gamma=10.0,     # 正则化损失的加权参数
            capacity=0.0,   # β-VAE的容量参数
            rand=0,         # 用于随机数生成器的随机种子
            delta=1.0,      # 参数，用于控制扩展
    ):
        ...
        self.attr_dict = DATASET_REG_TYPE_DICT[self.dataset_type]       # 获得属性的字典

        self.reverse_attr_dict = {
            v: k for k, v in self.attr_dict.items()  # .items可以返回可遍历的(键, 值)元组数组
        }
        self.metrics = {}           # 指标
        self.beta = beta
    ...
    def loss_and_acc_for_batch(self, batch, epoch_num=None, batch_num=None, train=True):
        ...
        :return: scalar loss value, scalar accuracy value
         ...
         计算批次的loss和精度
         必须以元组形式返回（loss，accuracy），精度可以为None
         ：param batch：torch变量
         ：param epoch_num：int，用于更改训练时间表
         ：param batch_num：int
         ：param train：bool，True是执行反向传递
         ：return：标量损失值，标量精度值
        """
        ...
        # compute and add regularization loss if needed     计算并添加正则化损失（如果需要）
        if self.use_reg_loss:
            reg_loss = 0.0
            if type(self.reg_dim) == tuple:
                for dim in self.reg_dim:
                    reg_loss += self.compute_reg_loss(      # 正则化损失
            ...
            if flag:
                ...
                self.writer.add_scalar('loss_split/dist_loss', (dist_loss / self.beta).item(), epoch_num)
                self.writer.add_scalar('loss_split/reg_loss', (reg_loss / self.gamma).item(), epoch_num)
        else:
            if flag:
        ...
        # compute accuracy      计算精度
        accuracy = self.mean_accuracy(
            weights=torch.sigmoid(outputs),     # 权重
            targets=inputs      # 目标
        )
    ...
    def eval_model(self, data_loader, epoch_num=0):
        if self.writer is not None:
            ...
            # 计算可解释性指标
            interp_metrics = compute_interpretability_metric(latent_codes, attributes, attr_list)
    ...
    def compute_representations(self, data_loader):
        ...
        attributes, attr_list = self._extract_relevant_attributes(attributes)   # 获得 属性 和 属性值列表
        return latent_codes, attributes, attr_list

    # 计算评价指标
    def compute_eval_metrics(self):
        """Returns the saved results as dict or computes them"""
        """以dict的形式返回保存的结果或计算它们"""

        results_fp = os.path.join(                          
            os.path.dirname(self.model.filepath),           # os.path.dirname() #去掉文件名,返回目录。获取当前目录，并组合成新目录
            'results_dict.json'
        )
        if os.path.exists(results_fp):
            with open(results_fp, 'r') as infile:           # r是读文件的模式打开一个文件对象
                self.metrics = json.load(infile)
        else:
            batch_size = 128
            _, _, data_loader = self.dataset.data_loaders(batch_size=batch_size)
            latent_codes, attributes, attr_list = self.compute_representations(data_loader)
            interp_metrics = compute_interpretability_metric(       # 计算可解释性指标
                latent_codes, attributes, attr_list
            )
            self.metrics = {
                "interpretability": interp_metrics          # 可解释性指标
            }
    ...
    def get_resnet_accuracy(self):
        if self.dataset_type != 'mnist':
            return None
        # instantiate Resnet model 实例化Resnet模型
        ...
        for sample_id, batch in tqdm(enumerate(data_loader)):
            ...
            # compute input and reconstruction accuracy on resnet 在resnet上计算输入和重建精度
    ...
    def create_latent_gifs(self, sample_id=9, num_points=10):
        x1 = torch.linspace(-4, 4.0, num_points)         # 从-4到4之间返回10个均匀的样本
        _, _, data_loader = self.dataset.data_loaders(batch_size=1)
        # _代表没有名字的变量，意思就是虽然函数有三个返回值，但是我这里只使用第三个返回值，前两个不用

        interp_dict = self.compute_eval_metrics()["interpretability"]   # 可解释性，计算评价指标
    ...
    def loss_and_acc_test(self, data_loader):
        ...
        for sample_id, batch in tqdm(enumerate(data_loader)):
            ...
            loss = recons_loss
            # compute mean loss and accuracy 计算平均损失和精度
